...
import { UserClassProps, UserClassState } from "../types/components";

class UserClass extends React.Component<UserClassProps, UserClassState> {
  constructor(props: UserClassProps) {
    super(props);
    console.log("child constructor method loading");
    this.state = {
      descInfo: {
  ...
  async componentDidMount() {
    ...
    try {
      ...
      console.log(
        "Fetched data in child componentDidMount",
        jsonData.data.cards[0].card.card
      );
      this.setState(
        {
